...
gene_pca = pca_genes %>% 
  ...
  #scale_size_continuous(limits = c(5, 15), breaks = seq(5, 15, by = 5)) +
  labs(
    x = glue("PC2 ({percent(pca_genes$sdev[[1]]^2 / sum(pca_genes$sdev^2), 0.1)})"),
    y = glue("PC3 ({percent(pca_genes$sdev[[2]]^2 / sum(pca_genes$sdev^2), 0.1)})"),
    colour = "Group",
    size = "Library Size\n(millions)"
...
```{r fig.cap="*Contribution of each GC/Length Bin to PC1 and PC2. Fill colours indicate the t-statistic, with tranparency denoting significance as -log10(p), using Bonferroni-adjusted p-values. GIven that the cell-line is clearly captured by PC1, the appearance of GC & length artefacts on PC2 is as expected*"}
mcols(transGR) %>%
  as.data.frame() %>%
